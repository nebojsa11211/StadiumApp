...
test.describe('Admin Polygon Drawing Tool - CORRECTED TEST', () => {
  ...
  test('should properly test polygon drawing with correct IDs and flow', async ({ page }) => {
    ...
    console.log('✓ Navigated to drawing tool page');
    ...
    console.log('✓ Canvas found with correct ID: admin-drawing-tool-canvas');

    // Get canvas dimensions
    const canvasBoundingBox = await canvas.boundingBox();
    expect(canvasBoundingBox).not.toBeNull();
    console.log(`✓ Canvas dimensions: ${canvasBoundingBox?.width}x${canvasBoundingBox?.height}`);
    ...
    console.log('✓ All required JavaScript functions exist');
    ...
    console.log('✓ Shape dropdown toggle found');
    ...
    console.log('✓ Polygon option found in dropdown');
    ...
    if (modalCount > 0) {
      console.log('✓✓✓ SUCCESS: Modal appeared after polygon drawing! ✓✓✓');
      ...
      console.log('✓ Screenshot saved: polygon-modal-success.png');
      ...
      console.error('❌ FAILURE: Modal did not appear after double-click');
  ...
  test('should test polygon cancellation with ESC key', async ({ page }) => {
    ...
    console.log('✓ No modal appeared (correct - polygon was cancelled)');
 ...
 * 5. Click dropdown button "➕ Create Sector (Rectangle)"
 * 6. Click "⬡ Custom Polygon (multi-click)" option
