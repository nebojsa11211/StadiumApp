...
/// <summary>
/// Staff-specific implementation of the unified token storage service
/// </summary>
public class StaffTokenStorageService : ITokenStorageService
{
    ...
    public string ApplicationContext => AuthenticationConstants.ApplicationContexts.Staff;
    ...
    public StaffTokenStorageService(IJSRuntime jsRuntime)
    {
    ...
    public async Task StoreTokenAsync(string token, DateTime? expiresAt = null, string? userEmail = null)
    {
        ...
        lock (_lock)
        {
            ...
            _tokenInfo = TokenInfo.Create(token, expiresAt ?? DateTime.UtcNow.AddHours(1), userEmail);
        ...
        try
        {
            await _jsRuntime.InvokeVoidAsync("localStorage.setItem",
                AuthenticationConstants.StorageKeys.Staff.Token, token);

            if (expiresAt.HasValue)
            {
                await _jsRuntime.InvokeVoidAsync("localStorage.setItem",
                    AuthenticationConstants.StorageKeys.Staff.TokenExpiration,
            ...
            {
                await _jsRuntime.InvokeVoidAsync("localStorage.setItem",
                    AuthenticationConstants.StorageKeys.Staff.Email, userEmail);
            }
        }
        catch (Exception)
        {
    ...
    public async Task<string?> GetTokenAsync()
    {
        ...
        try
        {
            var storedToken = await _jsRuntime.InvokeAsync<string?>("localStorage.getItem",
                AuthenticationConstants.StorageKeys.Staff.Token);
    ...
    public async Task<TokenInfo?> GetTokenInfoAsync()
    {
        ...
        try
        {
            ...
            var userEmail = await _jsRuntime.InvokeAsync<string?>("localStorage.getItem",
                AuthenticationConstants.StorageKeys.Staff.Email);
    ...
    public async Task ClearTokenAsync()
    {
        ...
        try
        {
            ...
            await _jsRuntime.InvokeVoidAsync("localStorage.removeItem",
                AuthenticationConstants.StorageKeys.Staff.Email);
            await _jsRuntime.InvokeVoidAsync("localStorage.removeItem",
                AuthenticationConstants.StorageKeys.Staff.RefreshToken);
            await _jsRuntime.InvokeVoidAsync("localStorage.removeItem",
                AuthenticationConstants.StorageKeys.Staff.RefreshTokenExpiration);
            await _jsRuntime.InvokeVoidAsync("localStorage.removeItem",
                AuthenticationConstants.StorageKeys.Staff.UserData);
        }
        catch (Exception)
        {
    ...
    public async Task<bool> IsTokenValidAsync()
    {
        ...
        return !JwtTokenValidator.IsTokenExpired(tokenInfo.Token);
    }
    ...
    public async Task StoreUserDataAsync(string key, string value)
    {
        ...
        var userDataKey = $"{AuthenticationConstants.StorageKeys.Staff.UserData}_{key}";
    ...
    public async Task ClearUserDataAsync()
    {
        try
        {
            ...
            var userDataPrefix = AuthenticationConstants.StorageKeys.Staff.UserData;
    ...
    private void UpdateTokenInfo()
    {
        ...
        _tokenInfo = TokenInfo.Create(_token, expiresAt ?? DateTime.UtcNow.AddHours(1), userEmail);
    ...
    public async Task StoreRefreshTokenAsync(string refreshToken, DateTime? expiresAt = null)
    {
        ...
        {
            await _jsRuntime.InvokeVoidAsync("localStorage.setItem",
                AuthenticationConstants.StorageKeys.Staff.RefreshToken, refreshToken);
    ...
    public async Task<string?> GetRefreshTokenAsync()
    {
        ...
        try
        {
            var storedRefreshToken = await _jsRuntime.InvokeAsync<string?>(
                "localStorage.getItem", AuthenticationConstants.StorageKeys.Staff.RefreshToken);
    ...
    public async Task<bool> IsRefreshTokenValidAsync()
    {
        ...
        try
        {
            var expirationStr = await _jsRuntime.InvokeAsync<string?>(
                "localStorage.getItem", AuthenticationConstants.StorageKeys.Staff.RefreshTokenExpiration);
