...
window.hoverTribune = function(tribuneCode) {
    try {
        const tribuneElements = document.querySelectorAll(`[data-stand*="${tribuneCode}"]`);
        tribuneElements.forEach(element => {
            element.style.filter = 'brightness(1.2) drop-shadow(0 0 10px rgba(59, 130, 246, 0.5))';
            element.style.transition = 'all 0.3s ease';
        });
        
        // Announce to screen readers
        if (window.stadiumViewer) {
            window.stadiumViewer.announceToScreenReader(`Highlighting ${tribuneCode} tribune`);
        }
    } catch (error) {
...
window.unhoverTribune = function() {
    try {
        const tribuneElements = document.querySelectorAll('[data-stand]');
        tribuneElements.forEach(element => {
            element.style.filter = '';
            element.style.transition = '';
        });
    } catch (error) {
        console.warn('Error removing tribune hover:', error);
    }
};

window.highlightTribune = function(tribuneCode) {
    try {
        // Remove previous highlights
        const allTribunes = document.querySelectorAll('[data-stand]');
        allTribunes.forEach(element => {
            element.classList.remove('tribune-highlighted');
        });
        
        // Add highlight to selected tribune
        const tribuneElements = document.querySelectorAll(`[data-stand*="${tribuneCode}"]`);
        tribuneElements.forEach(element => {
            element.classList.add('tribune-highlighted');
        });
        
        // Announce to screen readers
        if (window.stadiumViewer) {
            window.stadiumViewer.announceToScreenReader(`Selected ${tribuneCode} tribune`);
        }
    } catch (error) {
        console.warn('Error highlighting tribune:', error);
    }
};

window.renderSeatsOnCanvas = function(canvas, sectorData, eventId) {
    try {
        if (window.stadiumViewer) {
            return window.stadiumViewer.renderSeatsOnCanvas(canvas, sectorData, eventId);
        }
    } catch (error) {
        console.error('Error rendering seats on canvas:', error);
    }
};
